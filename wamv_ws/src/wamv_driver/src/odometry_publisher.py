...
class OdometryPublisher():
    ...
    def __init__(self):
        # Initialize node
        rospy.init_node('odometry_publisher_node', log_level = rospy.DEBUG)

        # Set up subscribers
        rospy.Subscriber('an_device/Twist', Twist, self.twist_callback)
        rospy.Subscriber('an_device/Imu', Imu, self.imu_callback)
        rospy.Subscriber('an_device/NavSatFix', NavSatFix, self.nav_sat_fix_callback)
    ...
    def nav_sat_fix_callback(self, msg):
        ...
        rospy.loginfo("Northing: " + str(northing) + " Easting: " + str(easting))
