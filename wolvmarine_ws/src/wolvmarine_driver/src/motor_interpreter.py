...
class MotorInterpreter():
    ...
    def __init__(self):
        ...
        self.port_motor_pub = rospy.Publisher(
            'port_motor_input', Int16, queue_size=100)
        self.strbrd_motor_pub = rospy.Publisher(
            'strbrd_motor_input', Int16, queue_size=100)
        self.port_thruster_pub = rospy.Publisher(
            'port_thruster_input', Int16, queue_size=100)
        self.strbrd_thruster_pub = rospy.Publisher(
            'strbrd_thruster_input', Int16, queue_size=100)

        self.system_mode = 'limbo'
        rospy.Subscriber('system_mode', Int16, self.mode_callback)
        rospy.Subscriber('motor_command', MotorCommand, self.command_callback)

        rospy.on_shutdown(self.publish_zero_speed)

    ###########################################################################
    def publish_zero_speed(self):
        self.port_motor_pub.publish(127)
        self.strbrd_motor_pub.publish(127)
        self.port_thruster_pub.publish(1500)
        self.strbrd_thruster_pub.publish(1500)
    ...
    def command_callback(self, msg):
        if self.system_mode == 'killed':
            ...
            self.port_thruster_pub.publish(msg.port_thruster)
            self.strbrd_thruster_pub.publish(msg.strbrd_thruster)
